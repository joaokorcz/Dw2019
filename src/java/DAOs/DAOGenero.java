...
public class DAOGenero extends DAOGenerico<Genero> {
    ...
    public List<Genero> listInOrderId_nome(String nome) {
        return em.createQuery("SELECT e FROM Genero e WHERE e.nomeGenero LIKE \"%" + nome + "%\" ORDER BY e.idGenero").getResultList();
    } 
    ...
    public List<String> listInOrderNomeStrings(String qualOrdem) {
        ...
        for (int i = 0; i < lf.size(); i++) {
            ls.add(lf.get(i).getIdGenero() + "-" + lf.get(i).getGenero());
        }
        return ls;
    }
}
